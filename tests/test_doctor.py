...
def test_doctor_report_shape():
    ...
    assert {"subfinder", "httpx", "nuclei", "dnsx", "katana", "naabu",
            "asnmap", "tlsx", "cdncheck"} <= set(rep["pd_tools"])
    assert {"gau", "ffuf", "dalfox", "trufflehog", "interactsh-client"} <= set(rep["extra_tools"])
